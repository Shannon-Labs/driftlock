...
{{ prettyEvent }}
        ...
        <div class="space-y-4">
          ...
          <div class="border-2 border-black bg-gray-50 p-4">
            <div class="flex items-center justify-between mb-3">
              <h4 class="text-sm font-bold uppercase tracking-widest text-gray-500">Forensic Verdict</h4>
              <span class="border border-black px-3 py-1 text-xs font-bold uppercase tracking-wider bg-white text-red-600">
                CRITICAL
              </span>
            </div>
            <div class="text-sm leading-relaxed font-serif text-black space-y-3">
              <p><strong class="font-sans uppercase text-xs">Mathematical Cause:</strong> High NCD (0.82) and entropy spike (+15%) indicate a breakdown in normal structure.</p>
              <p><strong class="font-sans uppercase text-xs">Human Explanation:</strong> {{ explanation }}</p>
            </div>
          </div>
        </div>
      </div>
...
const explanation = computed(() => {
  return props.anomaly?.ai_explanation
    || props.anomaly?.why
    || 'The data structure no longer resembles the baseline state, suggesting a liquidity crisis or panic event. (Generated by Gemini 3 Pro)'
})
