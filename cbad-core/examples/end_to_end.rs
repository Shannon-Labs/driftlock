...
use cbad_core::window::{DataEvent, SlidingWindow, WindowConfig};
use cbad_core::{
    compression::create_adapter, compression::CompressionAlgorithm, compute_metrics, ComputeConfig,
};
...
fn main() {
    ...
    let adapter =
        create_adapter(CompressionAlgorithm::Zstd).expect("Failed to create compression adapter");
    ...
    match compute_metrics(
        ...
        Ok(metrics) => {
            println!(
                "NCD Score: {:.3} (0.0 = identical, 1.0 = completely different)",
                metrics.ncd
            );
            println!("P-Value: {:.3} (statistical significance)", metrics.p_value);
            println!(
                "Compression Ratio - Baseline: {:.2}x, Window: {:.2}x",
                metrics.baseline_compression_ratio, metrics.window_compression_ratio
            );
            println!(
                "Entropy - Baseline: {:.2} bits/byte, Window: {:.2} bits/byte",
    ...
    for i in 0..15 {
        let event_data = if i < 10 {
            ...
            format!(
                r#"{{"event_id":{},"level":"INFO","service":"api-gateway","action":"request_completed","duration_ms":{}}}"#,
                i,
                40 + (i % 5)
            )
        } else {
            ...
            format!(
                r#"{{"event_id":{},"level":"ERROR","service":"api-gateway","action":"critical_error","exception":"stack_overflow"}}"#,
                i
            )
        };
    ...
    println!(
        "📊 Added {} events to sliding window",
        sliding_window.total_events()
    );
    ...
    match sliding_window.get_baseline_and_window() {
        Some((baseline, window)) => {
            println!(
                "✅ Retrieved baseline ({} bytes) and window ({} bytes) from sliding window",
                baseline.len(),
                window.len()
            );
            ...
            match compute_metrics(&baseline, &window, adapter.as_ref(), &config) {
                Ok(metrics) => {
                    ...
                    println!(
                        "   Anomaly: {}",
                        if metrics.is_anomaly { "YES" } else { "NO" }
                    );
                }
                Err(e) => {
    ...
    use cbad_core::performance::{BenchmarkConfig, DataType, PerformanceValidator};
    ...
    println!(
        "✅ Running performance validation ({}s duration)...",
        duration
    );
