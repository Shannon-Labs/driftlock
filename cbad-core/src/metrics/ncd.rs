...
impl NCDMetrics {
    ...
    pub fn concatenation_overhead(&self) -> f64 {
        let min_individual = self
            .baseline_compressed_size
            .min(self.window_compressed_size) as f64;
...
pub fn compute_ncd_detailed(
    ...
    let combined_compressed = adapter.compress(&combined).map_err(|e| {
        MetricsError::CompressionFailed(format!("Combined compression failed: {}", e))
    })?;
...
mod tests {
    ...
    fn test_ncd_identical_sequences() {
        ...
        let metrics = compute_ncd_detailed(data.as_slice(), data.as_slice(), adapter.as_ref())
    ...
    fn test_ncd_similar_sequences() {
        ...
        let metrics =
            compute_ncd_detailed(baseline.as_slice(), window.as_slice(), adapter.as_ref())
    ...
    fn test_ncd_dissimilar_sequences() {
        ...
        println!("Interpretation: {}", metrics.interpretation());
        println!(
            "Concatenation overhead: {:.1}%",
            metrics.concatenation_overhead() * 100.0
        );
    ...
    fn test_ncd_range() {
        ...
        let test_cases = vec![
            (
                b"short".as_slice(),
                b"very long sequence with lots of data".as_slice(),
            ),
        ...
        for (baseline, window) in test_cases {
            if !baseline.is_empty() && !window.is_empty() {
                let ncd = compute_ncd(baseline, window, adapter.as_ref()).expect("compute NCD");
    ...
    fn test_ncd_symmetry() {
        ...
        let seq2 =
            b"ERROR 2025-10-24T00:00:01Z service=api-gateway msg=stack_trace_panic\n".repeat(20);
    ...
    fn test_ncd_matrix() {
        ...
        let sequences = vec![seq1.as_slice(), seq2.as_slice(), seq3.as_slice()];

        let matrix = compute_ncd_matrix(&sequences, adapter.as_ref()).expect("compute NCD matrix");
    ...
    fn test_otlp_log_ncd() {
        ...
        assert!(
            baseline_similar < baseline_anomalous,
            "Similar logs should have lower NCD than anomalous logs"
        );

        // Anomalous log should be detectable as anomaly
        assert!(
            baseline_anomalous > 0.4,
            "Anomalous log should have high NCD"
        );
    }
    ...
    fn test_quick_anomaly_check() {
        ...
        let is_anomaly_low =
            is_anomaly_ncd(&baseline, &window, adapter.as_ref(), 0.9).expect("quick anomaly check");
        assert!(
            !is_anomaly_low,
        ...
        let is_anomaly_high =
            is_anomaly_ncd(&baseline, &window, adapter.as_ref(), 0.3).expect("quick anomaly check");
