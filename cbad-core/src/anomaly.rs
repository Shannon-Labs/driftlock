...
pub struct AnomalyConfig {
    ...
    /// Minimum entropy increase (fractional) to consider suspicious
...
impl AnomalyResult {
    ...
    pub fn new(metrics: AnomalyMetrics, p_value_threshold: f64) -> Self {
        let is_statistically_significant = metrics.p_value < p_value_threshold;
...
impl AnomalyDetector {
    ...
    pub fn detect_anomaly(
        ...
        // Apply anomaly detection criteria using configurable thresholds
        let ncd_pass = metrics.ncd >= self.config.ncd_threshold;
        let p_pass = metrics.p_value <= self.config.p_value_threshold;
        let compression_drop =
            ...
            AnomalyType::PatternBreak => {
                ...
                for _ in 0..count {
                    ...
                    let log_line = format!(
                        ...
                        "2025-10-24T12:00:00Z", stack_trace
                    );
                    events.push(log_line.as_bytes().to_vec());
                }
            }
...
mod tests {
    ...
    fn test_mixed_dataset_generation() {
        let (events, anomaly_indices) =
            synthetic::generate_mixed_dataset(100, 20, synthetic::AnomalyType::RandomNoise, 42);
    ...
    fn test_end_to_end_anomaly_detection() {
        ...
        let anomaly_events =
            synthetic::generate_anomaly(30, synthetic::AnomalyType::PatternBreak, 43);
        ...
        println!(
            ...
            anomaly_result.is_statistically_significant
        );
        ...
        assert!(
            ...
            "NCD should be at least {}, but got {}",
            0.2,
            anomaly_result.metrics.ncd
        );
        ...
        if anomaly_result.metrics.ncd >= 0.2 {
            ...
            println!(
                "High NCD detected ({} >= {}): Testing statistical significance",
                anomaly_result.metrics.ncd, 0.2
            );
        }
    }
    ...
    #[test]
    fn test_anomaly_result_respects_thresholds() {
        ...
        let result_relaxed = AnomalyResult::new(metrics.clone(), 0.2);
        assert!(result_relaxed.is_statistically_significant);

        let result_strict = AnomalyResult::new(metrics, 0.05);
        assert!(!result_strict.is_statistically_significant);
    }
}
