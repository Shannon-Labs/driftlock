...
use crate::metrics;
use crate::window::{DataEvent, SlidingWindow, WindowConfig};
...
impl PerformanceValidator {
    ...
    fn benchmark_compression_throughput(&self) -> BenchmarkResult {
        ...
        let adapter = create_adapter(Self::default_adapter()).expect("create adapter");
    ...
    fn benchmark_window_system(&self) -> BenchmarkResult {
        ...
        // Memory usage
        result.memory_mb =
            (window.memory_usage() * self.config.data_size) as f64 / (1024.0 * 1024.0);
    ...
    fn benchmark_end_to_end_detection(&self) -> BenchmarkResult {
        ...
        for _ in 0..1100 {
            // More than needed to ensure we have baseline and window
        ...
        while start_time.elapsed().as_secs() < self.config.duration {
            ...
            if let Some((baseline, detection_window)) = window.get_baseline_and_window() {
                let _metrics = crate::compute_metrics(
                    &baseline,
                    &detection_window,
                    adapter.as_ref(),
                    &compute_config,
        ...
        for result in results {
            ...
            if result.name == "End-to-End Anomaly Detection" {
                if result.ops_per_sec >= 10000.0 {
                    report.pass(
                        "Throughput",
                        format!("Achieved {:.0} ops/sec >= 10k", result.ops_per_sec),
                    );
                } else {
                    report.fail(
                        "Throughput",
                        format!("Achieved {:.0} ops/sec < 10k target", result.ops_per_sec),
                    );
                }
            }
            ...
            if result.name == "End-to-End Anomaly Detection" {
                ...
                if p95_ms < 400.0 {
                    ...
                    report.fail(
                        "Latency",
                        format!("P95 latency {:.1}ms >= 400ms target", p95_ms),
                    );
                }
            }
            ...
            // Memory usage check
            if result.name == "End-to-End Anomaly Detection" {
                if result.memory_mb < 2048.0 {
                    // 2GB
                    report.pass("Memory", format!("Used {:.1}MB < 2GB", result.memory_mb));
                } else {
                    report.fail(
                        "Memory",
                        format!("Used {:.1}MB >= 2GB target", result.memory_mb),
                    );
                }
            }
