...
func ComputeMetricsQuick(baseline []byte, window []byte) (*Metrics, error) {
	return ComputeMetrics(baseline, window, 42, 1000)
}
...
func (m *Metrics) GetAnomalyExplanation() string {
	...
	return fmt.Sprintf("Anomaly detected: NCD=%.3f, p=%.3f, compression ratio dropped from %.2fx to %.2fx due to data structure changes",
		m.NCD, m.PValue, m.BaselineCompressionRatio, m.WindowCompressionRatio)
}
...
func (m *Metrics) GetDetailedExplanation() string {
	...
	return fmt.Sprintf("CBAD Analysis: NCD=%.3f (%s), confidence=%.1f%%, baseline entropy=%.2f bits/byte, window entropy=%.2f bits/byte, compression change=%.1f%%",
		m.NCD, significance, m.ConfidenceLevel*100, m.BaselineEntropy, m.WindowEntropy,
