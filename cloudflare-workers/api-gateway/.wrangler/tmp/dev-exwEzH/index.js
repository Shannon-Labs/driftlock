...
// .wrangler/tmp/bundle-HFHKvp/checked-fetch.js
...
// .wrangler/tmp/bundle-HFHKvp/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-HFHKvp/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-HFHKvp/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
async function handleStripeWebhook(request, env, signature) {
  try {
...
async function handleCheckoutSessionCompleted(session, env) {
  ...
  const organizationId = session.client_reference_id || null;
  if (!organizationId) {
  ...
  const { error } = await supabase.from("subscriptions").update({
    ...
    status: "active",
    current_period_start: new Date(session.current_period_start * 1e3).toISOString(),
    current_period_end: new Date(session.current_period_end * 1e3).toISOString()
...
async function handleInvoicePaymentSucceeded(invoice, env) {
  const { error } = await supabase.from("invoices_mirror").update({
    status: "paid",
...
async function handleInvoicePaymentFailed(invoice, env) {
  const { error } = await supabase.from("invoices_mirror").update({ status: "failed" }).eq("stripe_invoice_id", invoice.id);
  if (error) {
    console.error("Error updating invoice:", error);
  }
...
async function getOrgIdFromStripeCustomer(stripeCustomerId) {
  const { data, error } = await supabase.from("billing_customers").select("organization_id").eq("stripe_customer_id", stripeCustomerId).single();
  if (error) {
...
__name(getOrgIdFromStripeCustomer, "getOrgIdFromStripeCustomer");
...
__name(mapStripePriceToPlan, "mapStripePriceToPlan");
async function handlePaymentFailure(stripeCustomerId, env) {
  const orgId = await getOrgIdFromStripeCustomer(stripeCustomerId);
  if (!orgId)
    return;
  console.log(`Payment failed for organization: ${orgId}`);
}
__name(handlePaymentFailure, "handlePaymentFailure");
async function validateApiKey(apiKey, env) {
  try {
    const { data, error } = await supabase.from("api_keys").select("organization_id").eq("key_hash", await hashApiKey(apiKey)).single();
    if (error) {
...
async function isSubscriptionValid(organizationId, env) {
  try {
    const { data, error } = await supabase.from("subscriptions").select("status").eq("organization_id", organizationId).in("status", ["active", "trialing"]).single();
    if (error || !data) {
      return false;
    }
...
__name(isSubscriptionValid, "isSubscriptionValid");
async function isMeteredRequest(request) {
  const url = new URL(request.url);
...
__name(isMeteredRequest, "isMeteredRequest");
async function recordUsage(organizationId, count, env) {
  try {
...
async function logAuditEvent(organizationId, request, response, env) {
  try {
