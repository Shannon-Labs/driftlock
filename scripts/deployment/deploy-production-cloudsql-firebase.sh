...
check_prerequisites() {
    ...
    for secret in "${required_secrets[@]}"; do
        if ! gcloud secrets describe "$secret" --project="$PROJECT_ID" &> /dev/null; then
            echo "❌ Secret '$secret' not found. Please run: ./scripts/deployment/setup-gcp-cloudsql-firebase.sh"
            exit 1
        fi
    done
    ...
    if ! gcloud sql instances describe driftlock-db --project="$PROJECT_ID" &> /dev/null; then
        echo "❌ Cloud SQL instance 'driftlock-db' not found. Please run: ./scripts/deployment/setup-gcp-cloudsql-firebase.sh"
        exit 1
    fi

    # Check if Firebase is initialized
    if ! firebase projects:list | grep -q "$PROJECT_ID"; then
        echo "❌ Firebase not initialized for project '$PROJECT_ID'. Please run: ./scripts/deployment/setup-gcp-cloudsql-firebase.sh"
        exit 1
    fi
